...
u8 Data_Receive_judge( u8 num, u8 code,u8 n , ThinkGearStreamParser *parser) ;
        ...
        switch(code)
		 {
			  ...
			  case PARSER_CODE_RAW_SIGNAL:
			  	...
			  	//printf("they are values of raw signal.\n");
...
/*
 * @return 1 if the @c code is somehow in an unrecognized code.
 * @return 0 on success.
 */
...
//	u8 m=0;
	u8 returnValue=0;
		...
		switch(code)
		 {
				case PARSER_CODE_POOR_QUALITY: 
					signal_value=parser->payload[n]; break;
				case PARSER_CODE_ATTENTION:
					attention_value=parser->payload[n]; break;
				case PARSER_CODE_MEDITATION:
					meditation_value=parser->payload[n]; break;
				case 0x16:
					blink_strength_value=parser->payload[n]; break;
				case PARSER_CODE_ASIC_EEG_POWER_INT:
		      //while(m<num)  EEG_power[m]=payload[n++];
		       break;
			  case PARSER_CODE_RAW_SIGNAL:
